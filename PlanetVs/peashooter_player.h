...
extern Atlas altas_peashooter_attack_ex_left;
extern Atlas altas_peashooter_attack_ex_right;
extern Atlas altas_peashooter_die_left;
extern Atlas altas_peashooter_die_right;
...
public:
	PeashooterPlayer(bool facing_right = true) : Player(facing_right) {
		...
		animation_attack_ex_left.set_atlas(&altas_peashooter_attack_ex_left);
		animation_attack_ex_right.set_atlas(&altas_peashooter_attack_ex_right);
		animation_die_left.set_atlas(&altas_peashooter_die_left);
		animation_die_right.set_atlas(&altas_peashooter_die_right);
...
private:
	void spawn_pea_bullet(float speed) {
		...
		const Vector2& bullet_size = bullet->get_size();//判断子弹根据脸部朝向调整坐标
		...
		//bullet->set_velocity(bullet_position.x, bullet_position.y);
		//傻逼...
